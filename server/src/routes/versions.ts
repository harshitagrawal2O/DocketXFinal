...
export async function saveVersion(documentId: string, name: string, auto: boolean, actor: Actor | null): Promise<VersionSummary> {
...
versionsRouter.get("/documents/:id/versions", async (req: AuthedRequest, res) => {
  ...
  const versions = await prisma.version.findMany({ where: { documentId: req.params.id }, orderBy: { createdAt: "desc" } });
...
versionsRouter.post("/documents/:id/versions", requireCap("manage_versions"), async (req: AuthedRequest, res) => {
  const { name } = req.body ?? {};
  const v = await saveVersion(req.params.id!, name || "Manual save", false, { userId: req.user!.id, name: req.user!.name });
  return res.json(v);
});
...
versionsRouter.get("/documents/:id/versions/:vid/text", async (req: AuthedRequest, res) => {
  const role = await getRole(req.params.id!, req.user!.id);
  if (!role) return res.status(403).json({ error: "Not a member" });
  const v = await prisma.version.findUniqueOrThrow({ where: { id: req.params.vid } });
...
versionsRouter.get("/documents/:id/versions/diff", async (req: AuthedRequest, res) => {
  ...
  const [fv, tv] = await Promise.all([
    prisma.version.findUniqueOrThrow({ where: { id: from } }),
    prisma.version.findUniqueOrThrow({ where: { id: to } }),
  ]);
  return res.json({
...
versionsRouter.post("/documents/:id/versions/:vid/rollback", requireCap("manage_versions"), async (req: AuthedRequest, res) => {
  ...
  const newVersion = await saveVersion(req.params.id!, `Rollback to "${v.name}"`, true, { userId: req.user!.id, name: req.user!.name });
